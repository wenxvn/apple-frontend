...
- `src/App.jsx` 新增位于探索器后的性能章节；桌面端“性能”导航可平滑定位到该章节，进入视口时以 GSAP 依次呈现芯片视觉和三项性能指标，且遵从减弱动态偏好。
- 程序化 Three.js 笔记本现使用圆角单体机身、刘海屏、细键盘、双侧扬声器孔、触控板与铰链；采用明亮中性布光和无环形图案的暖色渐变壁纸，深色与银色机身均可辨识。
- `src/styles.css` 定义首屏导航、Hero、下方探索器、性能章节、紧凑的胶囊式配置按钮、键盘可访问焦点和移动端响应式规则。
- `ui-registry.md` 已记录 Hero、产品探索配置器与性能展示的可复用视觉模式。
...
- 如需继续，可为“显示屏”导航新增与性能章节同等级的展示区；保持明亮的非环形屏幕壁纸、中性产品布光和深色沉浸式滚动节奏。
